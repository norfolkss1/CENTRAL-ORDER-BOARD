...
function embedVideoHtml(url) {
  ...
  if (yt) return `<div class="step-video"><iframe src="https://www.youtube.com/embed/${yt[1]}" frameborder="0" allowfullscreen></iframe></div>`;
  const vm = url.match(/vimeo\.com\/(\d+)/);
  if (vm) return `<div class="step-video"><iframe src="https://player.vimeo.com/video/${vm[1]}" frameborder="0" allowfullscreen></iframe></div>`;
  return `<div class="step-video"><video controls src="${escapeHtml(url)}"></video></div>`;
}
  ...
  row.innerHTML = `
    ...
    <div class="step-editor-media-row hidden" data-role="video-row">
      <div class="step-editor-video-input">
        <input class="field" placeholder="YouTube, Vimeo, or direct video URL" data-role="step-video" value="${escapeHtml(step.video || "")}">
      </div>
